...
- **Manual start.** The service is started by an explicit command. It survives
  ...
  after a reboot. This is achieved structurally — the rendered plist lives in
  `deploy/`, not in `~/Library/LaunchAgents/`, so launchd does not auto-load it
  at login.
...
The rendered plist is written to `deploy/org.laeser.app.plist` (in the repo,
gitignored), **not** to `~/Library/LaunchAgents/`. That directory auto-loads
plists at login; keeping ours out of it is what gives us "no reboot revival."
`laeserctl start` bootstraps the plist explicitly from its repo path.
...
- **`WorkingDirectory`** — the app uses relative paths for `laeser.db` and
  ...
  required.
- **`KeepAlive = true`** — while the job is *loaded*, launchd keeps it running
  (any exit, signal or otherwise, triggers a restart). Stop is achieved by
  *unloading* the job (`launchctl bootout`), not by signalling it — so the
  restart loop is interrupted by removing the job, not by exit-code conditions.
  This sidesteps the fact that uvicorn exits 143 on SIGTERM (indistinguishable
  from a crash if we tried to gate on exit code).
- **No `RunAtLoad`** — with `KeepAlive = true`, the key is redundant: launchd
  starts the job on bootstrap regardless.

The plist is committed to the repo as a **template**
(`deploy/org.laeser.app.plist.template`) with `__PLACEHOLDER__` tokens for the
machine-specific values (`__UV_PATH__`, `__PROJECT_DIR__`, `__LOG_DIR__`,
`__SERVICE_PATH__`), so no hardcoded home path enters version control. The
rendered `deploy/org.laeser.app.plist` is gitignored.
...
4. If a legacy plist exists at `~/Library/LaunchAgents/org.laeser.app.plist`
   (from a previous, buggy install), `bootout` the job and remove the file.
5. Render `org.laeser.app.plist.template` → `deploy/org.laeser.app.plist`,
   substituting the placeholder tokens. Does **not** bootstrap — `laeserctl
   start` does that, preserving manual start.
...
syntax. Control is by load state, not signals.
